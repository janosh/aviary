...
from aviary.segments import SimpleNet, WeightedAttentionPooling
...
class MessageLayer(nn.Module):
    ...
    def __init__(self, elem_fea_len, elem_heads, elem_gate, elem_msg):
        ...
        gate_nn = lambda: SimpleNet([2 * elem_fea_len, *elem_gate, 1])
        message_nn = lambda: SimpleNet([2 * elem_fea_len, *elem_msg, elem_fea_len])

        # Pooling and Output
        pooling_layers = [
            WeightedAttentionPooling(gate_nn(), message_nn()) for _ in range(elem_heads)
        ]
        self.pooling = nn.ModuleList(pooling_layers)
